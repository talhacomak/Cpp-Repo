...
class Operator { // main operator class
	int center_x;
	int center_y;
	int op_size;
	public:
...
class ArithmeticOperator: public Operator { // inherited class
	char sign;
	public:
        ...
        switch (cha) {
            ...
            case 'x':
                ...
                for (int i = 0; i < z; i++) {
                    a++;
                    b++;
                    if (a >= grid_rows || b >= grid_cols) break;
                    if (grid[a][b] != '\0') {
                        cout << "CONFLICT ERROR: Operator " << cha << " with size " << z << " can not be placed on ("
                             << x + 1 << "," << y + 1 << ")." << endl;
                        check = false;
            ...
            switch (operators[op_case]->get_sign()) {
                case '/' : //      '/' operator
                    ...
                    if (a + j >= row || b + j >= col || a - j < 0 || b - j < 0) {
                        ...
                        // checking for conflict
                    if (grid[a][b] != '\0' && !(a >= row || b >= col || a < 0 || b < 0)) {
